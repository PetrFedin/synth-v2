...
- The durable KPI semantic registry is append-only: `kpi_definition_versions`, `kpi_source_mapping_versions` and `kpi_definition_dependencies` must never be updated/deleted to rewrite history. Create a new immutable record/version instead.
...
- Definition history, source-mapping history and observation/run history are separate immutable concerns. Do not overload one table/version field to represent all three.
