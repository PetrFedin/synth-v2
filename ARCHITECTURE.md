...
Product Identity V2 is the only canonical Style/StyleVersion/Colorway/Size/SKU hierarchy. `catalog_skus` is **DEPRECATED compatibility state**, never a second Product Master. Some legacy technical/UI workspaces still have compatibility mutation/read paths, so it is inaccurate to claim that the table is already globally read-only. Those compatibility writes may preserve old workspace behavior only; they may not originate new Product semantics or new commercial truth.

Canonical new commercial writes are exclusively `Product Identity → ProductReadinessSnapshot → CommercialProductProjectionVersion → projection-backed CommercialPublication → ProductSku-exact PriceListVersion → BuyerCatalogVersion → ProductSku-exact Selection → WholesaleOrder → OrderCommitSnapshot`. Migration 075 fails closed if historical V1 flat CommercialPublication/PriceList attempts to mint fresh PriceListVersion/BuyerCatalogVersion. The P0.5/#121 branch extends that fail-closed chain through Selection/Order/OrderCommit in application/domain code and migration 076: fresh PostgreSQL Selection must cite a rich projection-backed BuyerCatalogVersion, exact ProductSku lines and one frozen Retail Door context; fresh Order must copy that submitted Selection without live price/catalog reconstruction; fresh OrderCommit must preserve the same immutable lineage. Historical V1/legacy rows remain readable exactly as stored and never receive guessed ProductSku IDs. `PUB-005` therefore remains OPEN/PARTIAL for `/v2/catalog/skus` mutation/workspace dependencies, legacy technical readers/adapters and explicitly bounded compatibility-only no-commercial-reader/single-line textual-SKU paths; those paths must not become new canonical commerce.
...
PR #118 closed the missing executable harness and its exact head passed all required repository workflows listed in section 2.6. PR #119 additionally guards ProductSku-exact buyer pricing at domain/HTTP/OpenAPI/PostgreSQL boundaries and extends same-environment PostgreSQL acceptance with negative bypass checks. The overall production contract remains PARTIAL because `COMM-LC-008`, the remaining `PRICE-009` depth, BuyerCatalog→Selection→OrderCommit live acceptance and later P0 slices remain open. An intended live acceptance environment must still execute the accepted gate before this slice can be called `PROD-PROVEN`.
...
### 7.4 Selection / Color × Size matrix — PARTIAL canonical cutover implemented, acceptance/UI depth pending

The canonical Selection path is BuyerCatalogVersion-backed and ProductSku-first. With the production PostgreSQL runtime's commercial publication reader wired, creating a fresh buyer Selection requires an exact rich projection-backed `BuyerCatalogVersion`, accepted Showroom access and a concrete active Retail Door; the Selection freezes Publication id, PriceListVersion id, BuyerCatalogVersion id/content hash/access grant, projection id/version/hash, readiness snapshot id, StyleVersion id and the buyer Retail Door snapshot/version. A fresh canonical Selection cannot be created from a historical flat BuyerCatalogVersion.

Canonical matrix mutation is `PUT /v2/selections/{selectionId}/matrix`. Each input row is `{ sku, productSkuId?, quantity, note? }`; server-controlled price, currency and catalogVersion remain forbidden. `productSkuId` is the canonical selector when display SKU is ambiguous. For rich rows, line identity/deduplication is ProductSku-first, so two distinct ProductSku variants may legally share the same display `sku` without collapsing. The service re-fetches the exact pinned BuyerCatalogVersion on matrix mutation and submit, revalidates immutable catalog/hash/projection/brand/shop/showroom context, resolves each exact ProductSku/StyleVersion/Colorway/SizeValue row and re-freezes price/currency/MOQ/availability. Submitted frozen lines are revalidated against that same catalog rather than a current mutable master.

Compatibility-only paths remain deliberately bounded. The no-commercial-reader in-memory/application path can still represent historical/legacy Selection behavior for compatibility testing, and the older single-line textual-SKU route remains a compatibility surface. They are not the production PostgreSQL canonical write path and must not acquire new ProductSku commercial semantics. Migration 076 independently rejects fresh PostgreSQL Selection writes that do not cite a valid rich BuyerCatalogVersion or whose exact ProductSku lines, commercial basis or Retail Door snapshot drift from it; historical legacy rows are not rewritten.

Automated ProductSku-collision coverage is added in `tests/selection-product-sku-identity.test.mjs`. Public-runtime/PostgreSQL `BuyerCatalogVersion → Selection → OrderCommitSnapshot` acceptance and complete browser Color × Size UX are still required before this slice can be called live-proven or UI-complete.

### 7.5 WholesaleOrder — IMPLEMENTED canonical inheritance / live acceptance pending

Order lifecycle and bilateral confirmation are executable. In the production runtime, fresh Order creation now requires a submitted canonical BuyerCatalog-backed Selection and revalidates that Selection against the exact pinned BuyerCatalogVersion before persistence. The Order copies immutable Publication/PriceList/BuyerCatalog/projection/readiness/StyleVersion lineage, Retail Door snapshot, ProductSku lines, quantity, price, currency and catalog compatibility fields; it does not re-resolve a mutable live catalog or accept client price as order truth. A caller cannot switch the Retail Door after Selection has pinned it. Legacy no-commercial-reader behavior remains compatibility-only and is not the canonical PostgreSQL new-write contract.

### 7.6 OrderCommitSnapshot — IMPLEMENTED canonical ProductSku lineage / live acceptance pending

The immutable OrderCommitSnapshot is the deal truth used by supply/economics. Before attachment/commit, the application revalidates the active trade relationship, open Showroom, accepted access grant, exact BuyerCatalogVersion basis, submitted Selection and Order lines. Rich line matching is ProductSku-first rather than textual-SKU-first, so two committed variants with the same display SKU remain separate immutable lines. Quantity, frozen price, currency, ProductSku/StyleVersion/Colorway/SizeValue identity, terms, bilateral acceptance and Retail Door snapshot must agree with the frozen chain.

Migration 076 adds forward-only PostgreSQL guards across fresh Selection, Order and OrderCommit writes and blocks direct/internal bypasses that omit or alter the canonical BuyerCatalog/Selection/ProductSku lineage. Historical legacy rows remain readable; they are not backfilled with guessed ProductSku identity. The supported runtime still requires an ACC-004 public-HTTP/PostgreSQL acceptance slice before any `PROD-PROVEN` claim.
...
### 12.5 Canonical BuyerCatalog → Selection → OrderCommit API — PARTIAL implementation, verification/live acceptance pending

`POST /v2/selections` remains the Selection creation mutation and now exposes optional-at-transport `retailDoorId` in the composed OpenAPI. In the canonical BuyerCatalog-backed path the domain requires that Retail Door and freezes its exact version/snapshot together with the immutable BuyerCatalogVersion basis. Omission is tolerated only for explicitly legacy compatibility contexts without a pinned BuyerCatalogVersion.

`PUT /v2/selections/{selectionId}/matrix` is the canonical ProductSku-aware buyer matrix mutation. Each row allows `productSkuId` in addition to display `sku`, `quantity` and optional note. It rejects client-controlled price/currency/catalogVersion and uses ProductSku identity for rich duplicate/collision handling. The existing single-line textual-SKU mutation is compatibility-only until its consumers are retired or migrated; it is not permitted to define new canonical ProductSku identity.

`POST /v2/selections/{selectionId}/submit` rechecks the same immutable BuyerCatalogVersion, content hash, commercial projection lineage, currency and every frozen ProductSku line before advancing the cycle. `POST /v2/orders` inherits those facts from the submitted Selection; any supplied `retailDoorId` must equal the pinned Selection door. Bilateral term acceptance remains the existing order lifecycle. Attaching the ready order creates exactly one immutable OrderCommitSnapshot and revalidates the same BuyerCatalog/Selection/Order chain before persistence. Exact idempotency replay returns the stored command result and cannot mint another economic/order-commit fact.

OpenAPI remains version `1.17.0`; the added ProductSku/Retail Door fields complete the current `/v2` contract rather than creating a parallel version.
...
- Migration `076_canonical_selection_order_commit_new_writes.sql` continues PUB-005 forward-only enforcement from BuyerCatalogVersion into Selection, Order and OrderCommit. Fresh Selection writes must cite a valid projection-backed rich BuyerCatalogVersion, preserve exact publication/price/catalog/projection/readiness/StyleVersion and Retail Door snapshot lineage, and persist unique exact ProductSku rows matching frozen variant/price/currency/MOQ/availability. Fresh Order writes require a submitted canonical Selection, copy exact ProductSku economics and recomputed total, and cannot switch immutable commercial/buyer context. Fresh OrderCommit writes require the matching canonical ready Order/Selection and exact ProductSku lines/terms/accepted parties/total. Canonical updates revalidate the same basis and cannot move it; historical legacy→legacy rows remain readable/updatable only inside their legacy semantics and are not rewritten or promoted into new canonical facts.
...
PR #118 implemented `npm run acceptance:product-commercialization` and is merged. It creates a fresh positive READY graph, continues it through exact Collection assortment/Showroom/buyer access, CommercialProductProjectionVersion, projection-backed CommercialPublication, ProductSku-exact PriceListVersion and BuyerCatalogVersion, and checks the immutable chain with both brand and shop actors against the same PostgreSQL target. Its exact head passed the dedicated supported-runtime/PostgreSQL workflow and the required CI listed in section 2.6. PR #119 converged exact ProductSku minor-price input plus PostgreSQL fail-closed new-write checks and is merged in `main@7e16256953659f1d507e9952e39fab49b416c60c`. Repository CI success remains `IMPLEMENTED` evidence, not an intended-environment `PROD-PROVEN` claim unless that environment is explicitly the accepted production-evidence target.

The #121 P0.5 implementation candidate extends canonical runtime and database lineage from BuyerCatalogVersion through Selection/WholesaleOrder/OrderCommitSnapshot, including ProductSku-first collision handling and Retail Door freeze. Domain/OpenAPI/migration/test code exists in the branch, but public-runtime ACC-004 slice-4 acceptance and green release-candidate evidence are still required before this paragraph may be upgraded beyond implementation-candidate status.
...
3. Readiness → Projection → Publication → PriceList → BuyerCatalog — public-runtime harness IMPLEMENTED and merged in #118; #119 additionally converges ProductSku-exact pricing/new-write guards. Intended-environment `PROD-PROVEN` evidence is still pending and the semantic contract remains PARTIAL until `COMM-LC-008` and remaining `PRICE-009` depth are closed.
4. BuyerCatalog → Selection → OrderCommit — IMPLEMENTATION CANDIDATE in #121; canonical application/domain/OpenAPI/PostgreSQL guards and focused ProductSku identity tests exist, but public-runtime/PostgreSQL ACC-004 slice-4 acceptance and green exact-head CI are still required.
5. OrderCommit → Supply → Shipment — PLANNED acceptance expansion after slice 4 is green.
...
| BuyerCatalog/Linesheet | IMPLEMENTED/PARTIAL | fresh writes require exact projection-backed Publication + ProductSku PriceList; historical V1 is read-only; complete variant-rich Showroom/Linesheet UI and intended live evidence remain |
| Color × Size matrix | PARTIAL | canonical ProductSku-first BuyerCatalog-backed mutation is implemented in #121 candidate; full public-runtime acceptance/browser UX and compatibility-route retirement remain |
| WholesaleOrder/OrderCommit | IMPLEMENTED/PARTIAL evidence | canonical BuyerCatalog/Selection/ProductSku inheritance and migration 076 guards are in #121 candidate; exact-head CI plus ACC-004 public-runtime proof remain |
| SupplyCommitment | IMPLEMENTED | expand physical acceptance after canonical OrderCommit slice is green |
...
| Full Product → Margin live acceptance | PARTIAL | #117 readiness + #118 READY→BuyerCatalog repository-runtime slices implemented; #121 slice-4 implementation candidate exists; intended-live evidence and downstream slices 4–7 remain |
...
| `ACC-004` | P0 | Live acceptance does not yet prove the connected Product → Margin spine | progressively prove canonical Product → Margin spine through public runtime/PostgreSQL slices | OPEN/PARTIAL — #117 implements both Product Readiness scenarios; #118 merged READY→Projection→projection-native Publication→PriceList→BuyerCatalog with separate brand/shop actors and green exact-head repository acceptance; #121 implements the BuyerCatalog→Selection→OrderCommit candidate, but slice-4 public-runtime/PostgreSQL proof and downstream physical/economics slices remain open |
| `PUB-005` | P0 | Some historical CatalogSku/UI/compatibility behavior remains flat-catalog oriented | projection-only variant-rich publication/buyer catalog and ProductSku-exact downstream new writes; no new flat-catalog product/publication/order truth | OPEN/PARTIAL — #119 forbids V1 publication/price history from minting fresh BuyerCatalog truth; #121 candidate removes the production PostgreSQL Selection/OrderCommit fallback by requiring rich BuyerCatalog/ProductSku/RetailDoor lineage and migration 076. Remaining debt: `/v2/catalog/skus` mutation/workspace dependencies, legacy technical readers/adapters, no-commercial-reader compatibility code and the textual-SKU single-line Selection compatibility route |
...
| Selection | exact BuyerCatalogVersion + Publication/PriceList/projection/readiness/StyleVersion basis + accepted access grant + exact frozen Retail Door snapshot/version + ProductSku-first selected lines with StyleVersion/Colorway/SizeValue, frozen quantity/price/currency/catalog compatibility data; submitted lines are revalidated against the same immutable catalog |
| WholesaleOrder | exact submitted Selection + inherited BuyerCatalog/commercial projection/Retail Door basis + exact ProductSku lines/economics + bilateral terms/acceptance; fresh canonical Order cannot re-resolve live catalog truth |
| OrderCommitSnapshot | exact agreed WholesaleOrder + submitted Selection + BuyerCatalogVersion + Retail Door + exact ProductSku line/price/quantity/terms/currency; immutable content hash; same display SKU may exist on multiple distinct ProductSku lines without identity collapse |
...
| 2026-09-08 | #119 / `fix/pub005-canonical-commercial-writes` → `main@7e16256953659f1d507e9952e39fab49b416c60c` | P0.4 convergence slice: forbid historical V1 CommercialPublication/PriceList from originating fresh buyer commercial truth; make price override exact `productSkuId + wholesalePriceMinor` with server-derived major price; validate exact frozen ProductSku line/hierarchy/terms; add migration 075 DB bypass guards and forward-only V1-history preservation tests; extend Product commercialization acceptance to exact ProductSku pricing. Legacy CatalogSku workspace writes remain explicitly open rather than hidden. | 2.6, 3.1, 6.4–6.6, 7.4, 12.4, 13, 15–17, 19–20 | MERGED; canonical BuyerCatalog new-write boundary established; no intended-live `PROD-PROVEN` claim; `PUB-005`, `COMM-LC-008`, `PRICE-009` remain OPEN/PARTIAL |
| 2026-09-12 | #121 / `feat/pub-005-canonical-selection-order-commit` | P0.5 candidate: continue immutable BuyerCatalogVersion lineage through ProductSku-first Selection, frozen Retail Door, WholesaleOrder and OrderCommitSnapshot; revalidate exact pinned catalog on mutations/submit/commit; support display-SKU collisions through ProductSku identity; expose Selection Retail Door/ProductSku matrix contract; add migration 076 direct-DB new-write guards and focused ProductSku collision test | 3.1, 7.4–7.6, 12.5, 13, 15–17, 19–20 | IMPLEMENTATION CANDIDATE; code/domain/OpenAPI/migration/test synchronized in draft PR; required exact-head Verify/PostgreSQL CI and ACC-004 slice-4 public-runtime acceptance must pass before merge/stronger evidence claims |
