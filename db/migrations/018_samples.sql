...
CREATE TABLE IF NOT EXISTS samples (
  ...
  CONSTRAINT samples_requested_context_check CHECK (
    status = 'draft' OR status = 'cancelled' OR (supplier_code IS NOT NULL AND due_at IS NOT NULL AND requested_at IS NOT NULL)
  ),
  CONSTRAINT samples_state_timestamps_check CHECK (
    ...
    OR (status = 'cancelled' AND received_at IS NULL AND decision_at IS NULL AND cancelled_at IS NOT NULL)
  ),
