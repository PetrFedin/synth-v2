    ...
    IF NEW.control_contract ? family_key THEN
      ...
      LOOP
        IF jsonb_typeof(item) <> 'object'
           ...
           OR COALESCE(item ->> 'scope', '') NOT IN ('OBSERVATION', 'BINDING') THEN
          RAISE EXCEPTION 'KPI control contract % entries require id, version and scope OBSERVATION|BINDING', family_key
            USING ERRCODE = '23514';
        END IF;
      END LOOP;
