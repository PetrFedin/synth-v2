...
-- PUB-005 forward-only cutover. Historical V1 commercial snapshots remain
-- immutable/readable exactly as stored; they are not rewritten and ProductSku
-- identity is never inferred from textual SKU. Fresh PriceList/BuyerCatalog
-- writes must originate from projection-backed V2 commercial truth.
CREATE FUNCTION assert_canonical_commercial_price(publication jsonb, price jsonb)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    FOR j IN 0..jsonb_array_length(pub_style -> 'colorways') - 1 LOOP
      ...
      FOR k IN 0..jsonb_array_length(pub_colorway -> 'skus') - 1 LOOP
        ...
        IF COALESCE(product_sku_id, '') = ''
           ...
           OR (price_sku - 'buyerUnitPrice' - 'buyerCurrency' - 'buyerMinimumOrderQuantity') IS DISTINCT FROM pub_sku THEN
          RAISE EXCEPTION 'BUYER_CATALOG_PRICE_HIERARCHY_MISMATCH' USING ERRCODE = '23514';
        END IF;
        ...
        IF price_line IS NULL
           ...
           OR price_sku -> 'buyerMinimumOrderQuantity' IS DISTINCT FROM price_line -> 'minimumOrderQuantity' THEN
          RAISE EXCEPTION 'BUYER_CATALOG_PRICE_HIERARCHY_MISMATCH' USING ERRCODE = '23514';
        END IF;
      END LOOP;
    END LOOP;
...
CREATE FUNCTION validate_canonical_price_list_new_write()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  PERFORM assert_canonical_commercial_price(publication.payload, NEW.payload);

  IF publication.commercial_projection_id IS NULL
     OR publication.brand_id IS DISTINCT FROM NEW.brand_id
     OR publication.currency IS DISTINCT FROM NEW.currency
     OR NEW.payload ->> 'id' IS DISTINCT FROM NEW.id
     OR NEW.payload ->> 'publicationId' IS DISTINCT FROM NEW.publication_id
     ...
     OR (NEW.payload ->> 'publishedAt')::timestamptz IS DISTINCT FROM NEW.published_at THEN
    RAISE EXCEPTION 'PRICE_LIST_CANONICAL_ROW_MISMATCH' USING ERRCODE = '23514';
  END IF;
...
CREATE FUNCTION validate_canonical_buyer_catalog_new_write()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  showroom showrooms%ROWTYPE;
  invitation showroom_invitations%ROWTYPE;
BEGIN
  SELECT * INTO publication
    FROM commercial_publications
   WHERE id = NEW.publication_id
  ...
  IF publication.id IS NULL OR price.id IS NULL THEN
    RAISE EXCEPTION 'BUYER_CATALOG_CANONICAL_ROW_MISMATCH' USING ERRCODE = '23514';
  END IF;

  -- Revalidate the commercial source before validating buyer-access topology.
  -- A historical V1 publication/PriceList remains readable history and cannot
  -- be misclassified as a malformed fresh BuyerCatalog merely because its old
  -- snapshot has no modern invitation row.
  PERFORM assert_canonical_commercial_price(publication.payload, price.payload);

  IF showroom.id IS NULL OR invitation.id IS NULL THEN
    RAISE EXCEPTION 'BUYER_CATALOG_CANONICAL_ROW_MISMATCH' USING ERRCODE = '23514';
  END IF;

  IF publication.commercial_projection_id IS NULL
     OR price.publication_id IS DISTINCT FROM publication.id
     OR publication.brand_id IS DISTINCT FROM NEW.brand_id
     OR price.brand_id IS DISTINCT FROM NEW.brand_id
     OR price.shop_id IS DISTINCT FROM NEW.shop_id
     OR publication.currency IS DISTINCT FROM NEW.currency
     OR price.currency IS DISTINCT FROM NEW.currency
     OR showroom.brand_id IS DISTINCT FROM NEW.brand_id
     OR showroom.collection_id IS DISTINCT FROM publication.collection_id
     OR showroom.status IS DISTINCT FROM 'open'
     OR invitation.showroom_id IS DISTINCT FROM NEW.showroom_id
     OR invitation.brand_id IS DISTINCT FROM NEW.brand_id
     OR invitation.shop_id IS DISTINCT FROM NEW.shop_id
     OR invitation.status IS DISTINCT FROM 'accepted'
     OR invitation.expires_at <= NEW.published_at
     OR NEW.payload ->> 'status' IS DISTINCT FROM 'published'
     OR NEW.payload ->> 'id' IS DISTINCT FROM NEW.id
     OR NEW.payload ->> 'publicationId' IS DISTINCT FROM NEW.publication_id
     OR NEW.payload ->> 'priceListVersionId' IS DISTINCT FROM NEW.price_list_version_id
     OR NEW.payload ->> 'brandId' IS DISTINCT FROM NEW.brand_id
     OR NEW.payload ->> 'shopId' IS DISTINCT FROM NEW.shop_id
     OR NEW.payload ->> 'showroomId' IS DISTINCT FROM NEW.showroom_id
     OR NEW.payload ->> 'accessGrantId' IS DISTINCT FROM NEW.access_grant_id
     OR NEW.payload ->> 'collectionId' IS DISTINCT FROM publication.collection_id
     OR NEW.payload ->> 'currency' IS DISTINCT FROM trim(NEW.currency)
     OR NEW.payload ->> 'contentHash' IS DISTINCT FROM NEW.content_hash
     OR (NEW.payload ->> 'publishedAt')::timestamptz IS DISTINCT FROM NEW.published_at
     OR NEW.payload -> 'commercialProjectionId' IS DISTINCT FROM price.payload -> 'commercialProjectionId'
     OR NEW.payload -> 'commercialProjectionVersionNo' IS DISTINCT FROM price.payload -> 'commercialProjectionVersionNo'
     OR NEW.payload -> 'commercialProjectionContentHash' IS DISTINCT FROM price.payload -> 'commercialProjectionContentHash'
     OR NEW.payload -> 'readinessSnapshotId' IS DISTINCT FROM price.payload -> 'readinessSnapshotId'
     OR NEW.payload -> 'styleVersionId' IS DISTINCT FROM price.payload -> 'styleVersionId'
     OR NEW.payload -> 'lines' IS DISTINCT FROM price.payload -> 'lines'
     OR NEW.payload -> 'styles' IS DISTINCT FROM price.payload -> 'styles' THEN
    RAISE EXCEPTION 'BUYER_CATALOG_CANONICAL_ROW_MISMATCH' USING ERRCODE = '23514';
  END IF;
...
CREATE TRIGGER price_list_000_canonical_new_write
BEFORE INSERT ON price_list_versions
FOR EACH ROW EXECUTE FUNCTION validate_canonical_price_list_new_write();

CREATE TRIGGER buyer_catalog_000_canonical_new_write
BEFORE INSERT ON buyer_catalog_versions
FOR EACH ROW EXECUTE FUNCTION validate_canonical_buyer_catalog_new_write();

COMMENT ON FUNCTION assert_canonical_commercial_price(jsonb, jsonb) IS
  'PUB-005: new price/catalog writes require projection-native ProductSku lineage and exact frozen terms/hierarchy; V1 snapshots remain deprecated read-only history.';
