...
-- Label the attributes a style carries, and say how many its category expects.
--
-- The dimensions projection named the value ("Оверсайз") but never the field ("Посадка"), so every
-- screen had to hardcode a label for each attribute it wanted to show and could show nothing else.
-- The attribute catalogue is in the database now, so the field's own name travels with the value and
-- a card can render any governed attribute without knowing about it in advance. The expected count is
-- the size of the set the category calls for, which is what makes "12 of 32 filled" answerable.
...
SELECT
  ...
  jsonb_build_object(
    ...
    'productFamily', category.attributes ->> 'product_family',
    'categoryAttributeExpected', COALESCE(expected.attribute_count, 0),
...
LEFT JOIN LATERAL (
  SELECT jsonb_object_agg(
           ...
           jsonb_build_object(
             ...
             'labelRu', definition.name_ru,
             'labelEn', definition.name_en,
...
LEFT JOIN LATERAL (
  ...
  WHERE category.attributes ->> 'product_family' IS NOT NULL
    AND category.attributes ->> 'product_family' = ANY (definition.applies_to)
) expected ON true
LEFT JOIN LATERAL (
