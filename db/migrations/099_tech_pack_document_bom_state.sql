...
-- Say why the materials are missing, as the measurements already do.
--
-- The document attached the bill of materials only when it was published, which is right — an
-- unpublished bill is still being argued about and a factory must not build from it. But the section
-- then rendered empty under its own heading, with nothing to say whether the bill was a draft, or
-- absent, or simply not loaded. A reader cannot act on an empty box.
--
-- The bill is joined whatever its state, so the document can name that state; only a published
-- bill's lines are projected, so what a factory reads is unchanged.
...
SELECT pack.tech_pack_code AS id,
   ...
   FROM tech_packs pack
     ...
     LEFT JOIN LATERAL ( SELECT jsonb_agg(jsonb_build_object('position', line."position", 'component', line.component, 'materialCode', line.material_code, 'materialType', line.material_type, 'unit', line.unit, 'quantity', line.quantity, 'wastePercent', line.waste_percent, 'grossQuantity', line.gross_quantity, 'unitCost', line.unit_cost_snapshot, 'currency', line.material_currency) ORDER BY line."position") AS lines
           FROM bom_lines line
