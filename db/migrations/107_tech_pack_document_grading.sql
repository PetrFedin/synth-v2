...
-- The document carries the grade rule, and says which cells depart from it.
--
-- A factory grading a pattern works from the rule, not from a column of numbers it has to
-- re-derive. And a cell that was typed over the rule is a deliberate exception: one that the
-- document does not distinguish is a rule nobody can trust, because the reader cannot tell whether
-- 110 at size 50 was intended or is the arithmetic coming out differently from what they expect.
--
-- Only the measurement point projection changes. The rest of the view is migration 104's, carried
...
CREATE OR REPLACE VIEW tech_pack_document_workspace AS
 SELECT pack.tech_pack_code AS id,
   ...
   FROM tech_packs pack
     ...
     LEFT JOIN boms bom ON bom.sku = pack.sku
     LEFT JOIN LATERAL ( SELECT jsonb_agg(jsonb_build_object('position', line."position", 'component', line.component, 'materialCode', line.material_code, 'materialType', line.material_type, 'unit', line.unit, 'quantity', line.quantity, 'wastePercent', line.waste_percent, 'grossQuantity', line.gross_quantity, 'unitCost', line.unit_cost_snapshot, 'currency', line.material_currency, 'placement', line.placement, 'isMain', line.is_main) ORDER BY line."position") AS lines
           FROM bom_lines line
          WHERE line.bom_id = bom.id AND bom.status = 'published'::text) materials ON true
     LEFT JOIN LATERAL ( SELECT 'live'::text AS source,
            ...
            ( SELECT jsonb_agg(jsonb_build_object('pointCode', point.point_code, 'name', point.name, 'description', point.description, 'toleranceMinus', point.tolerance_minus, 'tolerancePlus', point.tolerance_plus, 'gradeSteps', point.grade_steps, 'overrides', ( SELECT jsonb_agg(override.size_code) AS jsonb_agg FROM measurement_values override WHERE override.chart_id = point.chart_id AND override.point_code = point.point_code AND override.source = 'override'), 'values', ( SELECT jsonb_object_agg(value.size_code, value.value) AS jsonb_object_agg
