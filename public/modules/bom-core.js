(function installBomCore(root) {
  ...
  function risk(risks, code, severity, details) {
    risks.push(Object.freeze({ code, severity, details: Object.freeze(details || {}) }));
  }
  ...
  function buildRegistry(boms, catalogSkus) {
    ...
    return Object.freeze({
      ...
      summary: Object.freeze({
        total,
        draft: items.filter((item) => item.bom.status === 'draft').length,
        published: items.filter((item) => item.bom.status === 'published').length,
        ...
        averageTotalCost: total ? items.reduce((sum, item) => sum + (finite(item.bom.totalCost) || 0), 0) / total : 0,
      }),
    });
  }
