...
function sectionCard(title, children, buttonLabel, onButton) { const card = el('section', { className: 'card section' }); const tools = el('div', { className: 'toolbar' }); tools.append(el('h3',{text:title})); if(buttonLabel&&typeof onButton==='function'){const b=el('button',{className:'button small',text:buttonLabel,type:'button'});b.addEventListener('click',onButton);tools.append(b);} const stack=el('div',{className:'stack'}); children.forEach(child=>stack.append(child)); card.append(tools,stack); return card; }
function toolbar(title, buttonLabel, action) { const bar=el('div',{className:'toolbar'});bar.append(el('p',{className:'muted',text:title}));if(buttonLabel&&typeof action==='function'){const b=el('button',{className:'button primary',text:buttonLabel,type:'button'});b.addEventListener('click',action);bar.append(b);}return bar; }
...
function actionButton(label, fn, variant='', confirmText=''){const b=el('button',{className:`button small ${variant}`,text:label,type:'button'});b.addEventListener('click',()=>{if(confirmText&&!window.confirm(I18N.translate(confirmText)))return;runAction(async()=>{await fn();await reload();renderApp();toast(I18N.t('common.operationComplete'),'success');},b);});return b;}
...
async function runAction(action,button){if(state.busy||button.disabled)return;state.busy=true;const old=button.textContent;setButtonBusy(button,true,I18N.t('common.processing'));try{await action();}catch(error){toast(error.message,'error');if(!state.token)renderLogin(I18N.t('auth.sessionEnded'));}finally{state.busy=false;if(button.isConnected)setButtonBusy(button,false,old);}}
