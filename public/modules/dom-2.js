...
function clearSession(){state.token='';state.user=null;state.workspace=emptyWorkspace();state.notifications=[];state.notificationUnreadCount=0;window.catalogBulkSelection?.clear();window.SynthaWorkspaceController?.reset(state.workspace);window.SynthaNotificationController?.reset({items:[],nextCursor:null,unreadCount:0});sessionStorage.removeItem(TOKEN_KEY);}
