(function installOmnidataV14ComponentSystem(global){
  ...
  function applyDetached(root){observer?.disconnect?.();try{return apply(root)}finally{observer?.takeRecords?.();installObserver(true)}}
  function scheduleApply(){if(scheduled)return;scheduled=true;yieldToBrowser(()=>{scheduled=false;applyDetached(document)})}
  function installObserver(reconnect){if(!global.MutationObserver)return;if(observer&&!reconnect)return;if(observer&&reconnect){observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','aria-selected','aria-pressed']});return}observer=new global.MutationObserver((mutations)=>{if(mutations.some((mutation)=>mutation.addedNodes?.length||mutation.type==='attributes'))scheduleApply()});observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','aria-selected','aria-pressed']})}
