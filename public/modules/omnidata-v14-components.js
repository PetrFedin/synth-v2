(function installOmnidataV14ComponentSystem(global){
  ...
  function apply(root=document){stopObserving();try{ensureBodyClass();if(document.body?.dataset)document.body.dataset.synthaComponentSystem=BUILD;assignComponents(root);translateText(root);decorateAbbreviations(root);auditLanguage(root);auditComponents(root)}finally{startObserving()}}
