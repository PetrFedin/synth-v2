...
function renderPartners() {
  ...
  if (header.active === 'legal-entities') {
    const rows = legalEntityRows();
    return odPage(odText('\u041a\u043e\u043d\u0442\u0440\u0430\u0433\u0435\u043d\u0442\u044b \u0438 \u0434\u043e\u0441\u0442\u0443\u043f\u044b', 'Partners and access'), header, odRegistry({
      scope: 'od-legal-entities',
      rows,
      columns: [
        { label: odText('\u041a\u043e\u0434', 'Code'), value: item => item.entityCode },
        { label: odText('\u041e\u0440\u0433\u0430\u043d\u0438\u0437\u0430\u0446\u0438\u044f', 'Organisation'), value: item => item.orgName },
        { label: odText('\u041d\u0430\u0438\u043c\u0435\u043d\u043e\u0432\u0430\u043d\u0438\u0435', 'Name'), value: item => item.latestVersion?.nameRu || '\u2014' },
        { label: odText('\u0420\u0435\u043a\u0432\u0438\u0437\u0438\u0442\u044b', 'Requisites'), value: item => legalEntityRequisiteSummary(item) },
        { label: odText('\u0421\u0442\u0430\u0442\u0443\u0441', 'Status'), render: item => statusBadge(item.status) },
      ],
      inspector: item => odInspector({
        title: item.entityCode,
        subtitle: item.orgName,
        status: item.status,
        tabs: [odText('\u0420\u0435\u043a\u0432\u0438\u0437\u0438\u0442\u044b', 'Requisites')],
        fields: [
          { label: odText('\u041e\u0440\u0433\u0430\u043d\u0438\u0437\u0430\u0446\u0438\u044f', 'Organisation'), value: item.orgName },
          { label: odText('\u042e\u0440\u0438\u0441\u0434\u0438\u043a\u0446\u0438\u044f', 'Jurisdiction'), value: item.latestVersion ? (item.latestVersion.jurisdiction === 'RU' ? odText('\u0420\u043e\u0441\u0441\u0438\u044f', 'Russia') : odText('\u0417\u0430\u0440\u0443\u0431\u0435\u0436\u043d\u0430\u044f', 'Foreign')) : '\u2014' },
          { label: odText('\u041d\u0430\u0438\u043c\u0435\u043d\u043e\u0432\u0430\u043d\u0438\u0435', 'Name'), value: item.latestVersion?.nameRu || '\u2014' },
          { label: odText('\u0420\u0435\u043a\u0432\u0438\u0437\u0438\u0442\u044b', 'Requisites'), value: legalEntityRequisiteSummary(item) },
          { label: odText('\u0412\u0435\u0440\u0441\u0438\u044f \u0440\u0435\u043a\u0432\u0438\u0437\u0438\u0442\u043e\u0432', 'Requisites version'), value: item.latestVersion ? `v${item.latestVersion.versionNo}` : '\u2014' },
        ],
        actions: legalEntityActions(item),
      }),
    }));
  }
  if (header.active === 'compliance-documents') {
    const rows = complianceDocumentRows();
    return odPage(odText('Контрагенты и доступы', 'Partners and access'), header, odRegistry({
      scope: 'od-compliance-documents',
      rows,
      columns: [
        { label: odText('Номер', 'Number'), value: item => item.documentNumber },
        { label: odText('Тип', 'Type'), value: item => complianceDocumentTypeName(item.documentType) },
        { label: odText('Эмитент', 'Issuer'), value: item => legalEntityLabel(item.organisationId, item.issuerLegalEntityId) },
        { label: 'ЭДО', value: item => item.edoStatus ? statusLabel(item.edoStatus) : '—' },
        { label: odText('Статус', 'Status'), render: item => statusBadge(item.status) },
      ],
      inspector: item => odInspector({
        title: item.documentNumber,
        subtitle: complianceDocumentTypeName(item.documentType),
        status: item.status,
        tabs: [odText('Документ', 'Document')],
        fields: [
          { label: odText('Организация', 'Organisation'), value: item.orgName },
          { label: odText('Эмитент', 'Issuer'), value: legalEntityLabel(item.organisationId, item.issuerLegalEntityId) },
          { label: odText('Контрагент', 'Counterparty'), value: item.counterpartyLegalEntityId ? legalEntityLabel(item.organisationId, item.counterpartyLegalEntityId) : '—' },
          { label: 'ЭДО', value: item.edoStatus ? statusLabel(item.edoStatus) : '—' },
