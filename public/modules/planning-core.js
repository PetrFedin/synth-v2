(function installPlanningCore(root) {
  ...
  function validDate(value) { if (!value) return null; const date = new Date(value); return Number.isNaN(date.valueOf()) ? null : date; }
  function clamp(value, min, max) { return Math.min(max, Math.max(min, value)); }
  function ratio(numerator, denominator) { return denominator > 0 ? clamp(numerator / denominator, 0, 1) : 0; }
  function addRisk(risks, code, severity, details) { risks.push(Object.freeze({ code, severity, details })); }
  ...
  function assessCampaign(workspace, campaign, now = new Date()) {
    ...
    const orders = list(workspace.orders).filter((item) => item.campaignId === campaign.id
    ...
    return Object.freeze({
      campaign, collections, skus, showrooms, cycles, selections, orders, gateScores, readiness,
      risks: Object.freeze(risks), highestRisk: risks[0]?.severity || 'low', overdue,
      daysToStart: startsAt ? Math.ceil((startsAt - nowDate) / DAY_MS) : null,
      daysToEnd: endsAt ? Math.ceil((endsAt - nowDate) / DAY_MS) : null,
      counts: Object.freeze({ collections: collections.length, publishedCollections: publishedCollections.length, skus: skus.length, publishedSkus: publishedSkus.length, showrooms: showrooms.length, openShowrooms: openShowrooms.length, cycles: cycles.length, selections: selections.length, orders: orders.length }),
    });
  }
  ...
  function buildPortfolio(workspace = {}, now = new Date()) {
    ...
    campaigns.sort((left, right) => (RISK_RANK[right.highestRisk] - RISK_RANK[left.highestRisk]) || left.readiness - right.readiness || String(left.campaign.name || left.campaign.id).localeCompare(String(right.campaign.name || right.campaign.id)));
    const total = campaigns.length;
    return Object.freeze({
      campaigns: Object.freeze(campaigns),
      summary: Object.freeze({
        total,
        active: campaigns.filter((item) => !['closed', 'cancelled'].includes(item.campaign.status)).length,
        averageReadiness: total ? Math.round(campaigns.reduce((sum, item) => sum + item.readiness, 0) / total) : 0,
        criticalCampaigns: campaigns.filter((item) => item.highestRisk === 'critical').length,
        overdueCampaigns: campaigns.filter((item) => item.overdue).length,
        upcoming30Days: campaigns.filter((item) => item.daysToStart !== null && item.daysToStart >= 0 && item.daysToStart <= 30).length,
        riskCount: campaigns.reduce((sum, item) => sum + item.risks.length, 0),
      }),
    });
  }
