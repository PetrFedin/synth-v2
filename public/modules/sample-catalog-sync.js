(function installSampleCatalogSync(global) {
  ...
  function actorKey() { return String(state.user?.actorId || state.user?.id || state.user?.email || 'anonymous'); }
  function resetForActor(key) { sync.actorKey = key; sync.active = false; sync.status = 'idle'; sync.error = ''; sync.generation += 1; }
  ...
  function stateView({ error = false } = {}) {
    ...
    if (error) {
      ...
      retry.type = 'button'; retry.className = 'primary'; retry.textContent = text('Повторить загрузку', 'Retry');
  ...
  renderView = (...args) => {
    ...
    if (state.view !== 'samples') { sync.active = false; return previousRenderView(...args); }
    if (!sync.active) { sync.active = true; sync.status = 'idle'; sync.error = ''; sync.generation += 1; }
