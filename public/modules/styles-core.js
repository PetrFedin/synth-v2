(function installStylesCore(root) {
  ...
  function styleUsage(workspace, sku) {
    ...
    const selectedUnits = selections.reduce((sum, selection) => sum + list(selection.lines).filter((line) => line.sku === sku.sku).reduce((inner, line) => inner + Math.max(0, finite(line.quantity) || 0), 0), 0);
    const orderedUnits = orders.reduce((sum, order) => {
      const lines = list(order.lines);
      if (lines.length) return sum + lines.filter((line) => line.sku === sku.sku).reduce((inner, line) => inner + Math.max(0, finite(line.quantity) || 0), 0);
      const source = selections.find((selection) => selection.id === order.selectionId);
      return sum + list(source?.lines).filter((line) => line.sku === sku.sku).reduce((inner, line) => inner + Math.max(0, finite(line.quantity) || 0), 0);
    }, 0);
  ...
  function assessStyle(workspace, sku) {
    ...
    const risks = [];
    const identityValid = typeof sku.sku === 'string' && /^[A-Z0-9][A-Z0-9._-]{1,63}$/.test(sku.sku) && typeof sku.name === 'string' && sku.name.trim().length >= 2;
    ...
    const gateScores = Object.freeze({ identity: identityValid ? 15 : 0, collectionContext: contextValid ? 15 : 0, commercialTerms: commercialValid ? 20 : 0, inventory: inventoryValid ? 20 : 0, publication: publicationValid ? 15 : 0, salesEnablement: openShowrooms.length ? 15 : showrooms.length ? 8 : 0 });
    ...
    return Object.freeze({ sku, collection, showrooms: Object.freeze(showrooms), openShowrooms: Object.freeze(openShowrooms), usage, gateScores, readiness, risks: Object.freeze(risks), highestRisk: risks[0]?.severity || 'low', blocking, ats, saleReady: readiness >= 80 && !blocking });
  }

  function buildRegistry(workspace = {}) {
    const styles = list(workspace.catalogSkus).map((sku) => assessStyle(workspace, sku));
    styles.sort((left, right) => (RISK_RANK[right.highestRisk] - RISK_RANK[left.highestRisk]) || left.readiness - right.readiness || String(left.sku.sku).localeCompare(String(right.sku.sku)));
    const total = styles.length;
    return Object.freeze({
      styles: Object.freeze(styles),
      summary: Object.freeze({
        total,
        published: styles.filter((item) => item.sku.status === 'published').length,
        draft: styles.filter((item) => item.sku.status !== 'published').length,
        saleReady: styles.filter((item) => item.saleReady).length,
        critical: styles.filter((item) => item.highestRisk === 'critical').length,
        averageReadiness: total ? Math.round(styles.reduce((sum, item) => sum + item.readiness, 0) / total) : 0,
        commerciallyUsed: styles.filter((item) => item.usage.selections.length || item.usage.orders.length).length,
        lowAts: styles.filter((item) => item.risks.some((candidate) => candidate.code === 'ATS_BELOW_MOQ')).length,
      }),
    });
  }
