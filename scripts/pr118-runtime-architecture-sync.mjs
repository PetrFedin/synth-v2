...
function replaceOnce(path, before, after) {
  ...
  if (first < 0) throw new Error(`Sync anchor missing in ${path}: ${before.slice(0, 120)}`);
  if (text.indexOf(before, first + before.length) >= 0) throw new Error(`Sync anchor ambiguous in ${path}: ${before.slice(0, 120)}`);
  fs.writeFileSync(path, `${text.slice(0, first)}${after}${text.slice(first + before.length)}`, 'utf8');
}

replaceOnce(
  'src/modules/measurements/public.mjs',
...
replaceOnce(
  'ARCHITECTURE.md',
  'canonical Measurement now consumes only normalized versioned runtime MDM `translations` instead of source JSON aliases',
  'canonical Measurement now consumes the exact persisted version snapshot shape (`translations.ru/en` strings plus versioned `attributes`) instead of source JSON aliases',
);

console.log('PR #118 runtime and ARCHITECTURE.md synchronization patch applied.');
