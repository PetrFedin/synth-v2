...
const LEGACY_WORKSPACE_STYLES=['/industrial-product.css','/bom.css','/measurements.css','/samples.css','/sourcing.css'];
...
const ODS_NATIVE_STYLES=['/tech-packs.css','/production-executions.css','/production-orders.css','/final-quality.css'];
...
for(const workspace of LEGACY_WORKSPACE_STYLES)assert(stylesheets.indexOf(workspace)<stylesheets.length-1,`Legacy workspace stylesheet must load below ODS: ${workspace}`);
...
console.log(`Omnidata Design System v1 contract OK (${ROLES.length} roles, ${PARTS.length} semantic parts, ${styledParts.size} styled parts, ${TOKENS.length} tokens; Tech Packs, Production Executions, Production Orders and Final Quality ODS-native).`);
