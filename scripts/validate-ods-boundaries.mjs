...
for(const retired of ['/tech-packs.css','/production-executions.css','/production-orders.css','/final-quality.css']){assert(!stylesheets.includes(retired),`${retired} must remain ODS-native and must not restore a local stylesheet.`);assert(!staticHandler.includes(`'${retired}':`),`${retired} local stylesheet route must not be restored.`)}
...
assert(presentLegacyDebt.length===19,`Expected 19 loaded legacy styles after four ODS-native migrations, found ${presentLegacyDebt.length}.`);
...
console.log(`ODS boundary contract OK (${presentLegacyDebt.length}/${LEGACY_VISUAL_DEBT.length} frozen legacy styles remain; Tech Packs, Production Executions, Production Orders and Final Quality are ODS-native; no new stylesheet or dynamic style API).`);
