...
for (const required of [
  ...
  if (!sources.includes(required)) fail(`Required Omnidata runtime is missing: ${required}`);
}
...
function assertRequiredOrder(actual, required) {
  ...
  for (const source of required) {
    ...
    if (index === -1) fail(`Required UI asset is missing: ${source}`);
...
function decodeUtf8(buffer, file) {
  ...
  try {
    ...
    fail(`Invalid UTF-8 source detected: ${path.relative(root, file)}`);
  }
