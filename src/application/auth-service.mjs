...
const TOKEN_PATTERN = /^swv2_[A-Za-z0-9_-]{43}$/;
...
export function createAuthService({
  ...
  invariant(typeof clock === 'function', 'AUTH_CLOCK_INVALID', 'Authentication clock is required');
  invariant(typeof nextId === 'function', 'AUTH_ID_GENERATOR_INVALID', 'Authentication id generator is required');
  ...
  return Object.freeze({
    ...
    async authenticate(token) {
      ...
      return store.transaction(async (tx) => {
        ...
        const now = currentTimestamp(clock);
        if (!session || session.status !== 'active' || !isFutureTimestamp(session.expiresAt, now)) return null;
    ...
    async logout(token) {
      ...
      const revokedAt = currentTimestamp(clock);
      return store.transaction(async (tx) => {
        if (typeof tx.revokeSessionByTokenHash === 'function') return tx.revokeSessionByTokenHash(tokenHash, revokedAt);
        const session = await tx.getSessionByTokenHash(tokenHash);
        if (!session || session.status !== 'active') return false;
        await tx.saveSession(Object.freeze({ ...session, status: 'revoked', revokedAt }));
        return true;
      });
    },
...
function isAccessToken(token) { return typeof token === 'string' && TOKEN_PATTERN.test(token); }
...
function currentTimestamp(clock) {
  let value;
  try { value = clock(); }
  catch { invariant(false, 'AUTH_CLOCK_INVALID', 'Authentication clock failed'); }
  invariant(validTimestamp(value), 'AUTH_CLOCK_INVALID', 'Authentication clock must return a valid timestamp');
  return new Date(Date.parse(value)).toISOString();
}
function validTimestamp(value) { return typeof value === 'string' && Number.isFinite(Date.parse(value)); }
function isFutureTimestamp(value, now) { return validTimestamp(value) && Date.parse(value) > Date.parse(now); }
