...
export function createBomQueryService({ reader } = {}) {
  ...
  return Object.freeze({
    async pageForActor(actorId, options = {}) {
      ...
      const filters = Object.freeze({ q: optionalSearch(options.q), status: optionalStatus(options.status), brandId: optionalIdentifier(options.brandId) });
      const scope = JSON.stringify([filters.q ?? null, filters.status ?? null, filters.brandId ?? null]);
      const afterSku = options.cursor === undefined || options.cursor === null || options.cursor === '' ? undefined : decodeBomCursor(options.cursor, { scope }).sku;
