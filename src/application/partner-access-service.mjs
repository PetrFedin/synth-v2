...
export function createPartnerAccessService({
  ...
  return Object.freeze({
    requestRelationship(commandId, actorId, { brandId, shopId }) {
      return execute(
        commandId,
        `requestRelationship:${actorId}:${brandId}:${shopId}`,
        actorId,
        async (tx) => {
          ...
          return Object.freeze({ membership, existing });
        },
        async (tx, { membership, existing }) => {
    ...
    acceptRelationship(commandId, actorId, relationshipId) {
      return execute(
        commandId,
        `acceptRelationship:${actorId}:${relationshipId}`,
        actorId,
        async (tx) => {
          ...
          return Object.freeze({ current, responderOrganisationId });
        },
        async (tx, { current, responderOrganisationId }) => {
    ...
    rejectRelationship(commandId, actorId, relationshipId) {
      return execute(
        commandId,
        `rejectRelationship:${actorId}:${relationshipId}`,
        actorId,
        async (tx) => {
          const current = requireEntity(await tx.getRelationship(relationshipId), 'RELATIONSHIP_NOT_FOUND', { relationshipId });
          const responderOrganisationId = current.requestedByOrganisationId === current.brandId ? current.shopId : current.brandId;
    ...
    revokeRelationship(commandId, actorId, relationshipId) {
      return execute(
        commandId,
        `revokeRelationship:${actorId}:${relationshipId}`,
        actorId,
        async (tx) => {
          const current = requireEntity(await tx.getRelationship(relationshipId), 'RELATIONSHIP_NOT_FOUND', { relationshipId });
          ...
          return Object.freeze({ current, membership });
        },
        async (tx, { current, membership }) => {
    ...
    inviteShopToShowroom(commandId, actorId, { showroomId, shopId, expiresAt }) {
      return execute(
        commandId,
        `inviteShopToShowroom:${actorId}:${showroomId}:${shopId}:${expiresAt}`,
        actorId,
        async (tx) => {
    ...
    acceptShowroomInvitation(commandId, actorId, invitationId) {
      return execute(
        commandId,
        `acceptShowroomInvitation:${actorId}:${invitationId}`,
        actorId,
        async (tx) => {
    ...
    declineShowroomInvitation(commandId, actorId, invitationId) {
      return execute(
        commandId,
        `declineShowroomInvitation:${actorId}:${invitationId}`,
        actorId,
        async (tx) => {
          const current = requireEntity(await tx.getShowroomInvitation(invitationId), 'SHOWROOM_INVITATION_NOT_FOUND', { invitationId });
          const membership = await tx.getMembership(current.shopId, actorId);
    ...
    revokeShowroomInvitation(commandId, actorId, invitationId) {
      return execute(
        commandId,
        `revokeShowroomInvitation:${actorId}:${invitationId}`,
        actorId,
        async (tx) => {
          const current = requireEntity(await tx.getShowroomInvitation(invitationId), 'SHOWROOM_INVITATION_NOT_FOUND', { invitationId });
