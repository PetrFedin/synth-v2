  ...
  return Object.freeze({
    submitClaim(commandId, actorId, receiptDiscrepancySnapshotId, input) {
      ...
      return store.transaction(async (tx) => {
        ...
        const discrepancy = requireEntity(await tx.lockDiscrepancy(receiptDiscrepancySnapshotId), 'RECEIPT_DISCREPANCY_NOT_FOUND', { receiptDiscrepancySnapshotId });
        ...
        const claim = createReceiptDiscrepancyClaimSnapshot({
          id: nextId('receipt-claim'),
          discrepancy,
    ...
    resolveClaim(commandId, actorId, claimSnapshotId, input) {
      ...
      return store.transaction(async (tx) => {
        const previous = await tx.getCommand(commandId);
        if (previous) invariant(fingerprintsMatch(previous.fingerprint, fingerprint), 'COMMAND_ID_CONFLICT', 'commandId was already used by another mutation', { commandId });
        const claim = requireEntity(await tx.lockClaim(claimSnapshotId), 'RECEIPT_CLAIM_NOT_FOUND', { claimSnapshotId });
        ...
        const resolution = createReceiptClaimResolutionSnapshot({
          id: nextId('receipt-claim-resolution'),
          claim,
    ...
    getClaimForActor(actorId, claimSnapshotId) {
      return store.transaction(async (tx) => {
        const claim = requireEntity(await tx.getClaim(claimSnapshotId), 'RECEIPT_CLAIM_NOT_FOUND', { claimSnapshotId });
    ...
    getResolutionForActor(actorId, resolutionSnapshotId) {
      return store.transaction(async (tx) => {
        const resolution = requireEntity(await tx.getResolution(resolutionSnapshotId), 'RECEIPT_CLAIM_RESOLUTION_NOT_FOUND', { resolutionSnapshotId });
