...
function schemas() {
  ...
  const editable = {
    ...
    logisticsCost: quantity,
    otherCost: quantity,
    notes: { oneOf: [{ type: 'string', minLength: 1, maxLength: 2000 }, { type: 'null' }] },
  };
  const required = Object.keys(editable);
  return {
    BomLineInput: lineInput,
    BomCreate: { type: 'object', additionalProperties: false, required: ['sku', ...required], properties: { sku: { type: 'string', pattern: SKU_PATTERN }, ...editable } },
    BomUpdate: { type: 'object', additionalProperties: false, required: ['expectedVersion', ...required], properties: { expectedVersion: { type: 'integer', minimum: 1, maximum: 2_147_483_647 }, ...editable } },
    ...
    BomLine: {
      ...
      properties: {
        ...
        materialType: { type: 'string', enum: ['fabric', 'trim', 'packaging', 'other'] }, unit: { type: 'string', enum: ['m', 'kg', 'pc', 'yd'] },
        quantity: positiveQuantity, wastePercent: { ...quantity, maximum: 1000 }, grossQuantity: positiveQuantity,
        materialCurrency: currency, unitCostSnapshot: positiveQuantity, exchangeRate: positiveQuantity, lineCost: quantity,
      },
    },
    Bom: {
      type: 'object', additionalProperties: false,
      ...
      properties: {
        ...
        materialCost: quantity, laborCost: quantity, overheadCost: quantity, logisticsCost: quantity, otherCost: quantity, totalCost: positiveQuantity,
        notes: editable.notes, status: { type: 'string', enum: ['draft', 'published'] }, version: { type: 'integer', minimum: 1 },
    ...
    BomPage: { type: 'object', additionalProperties: false, required: ['items', 'nextCursor'], properties: { items: { type: 'array', maxItems: 200, items: { $ref: '#/components/schemas/Bom' } }, nextCursor: { oneOf: [{ type: 'string', minLength: 1, maxLength: 2048 }, { type: 'null' }] } } },
  };
}
...
function paths() {
  return {
    '/boms': {
      ...
      post: { operationId: 'createBom', security: [{ bearerAuth: [] }], parameters: [idempotencyHeader], requestBody: body('#/components/schemas/BomCreate'), responses: mutationResponses('Created BOM') },
    },
    '/boms/{sku}': {
      get: { operationId: 'getBom', security: [{ bearerAuth: [] }], parameters: [skuParameter], responses: { 200: dataResponse('BOM', '#/components/schemas/Bom'), 400: errorResponse, 401: errorResponse, 403: errorResponse, 404: errorResponse } },
      patch: { operationId: 'updateBom', security: [{ bearerAuth: [] }], parameters: [skuParameter, idempotencyHeader], requestBody: body('#/components/schemas/BomUpdate'), responses: mutationResponses('Updated BOM') },
    },
    '/boms/{sku}/publish': {
      post: { operationId: 'publishBom', security: [{ bearerAuth: [] }], parameters: [skuParameter, idempotencyHeader], requestBody: body('#/components/schemas/BomVersionExpectation'), responses: mutationResponses('Published BOM') },
    },
  };
}
function mutationResponses(description) { return { 200: dataResponse(description, '#/components/schemas/Bom'), 400: errorResponse, 401: errorResponse, 403: errorResponse, 404: errorResponse, 409: errorResponse, 422: errorResponse }; }
