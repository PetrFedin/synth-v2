...
function schemas() {
  ...
  return {
    ...
    TechPack: { type: 'object', additionalProperties: false, required: ['id','techPackCode','sku','brandId','skuVersion','revision','sourceTechPackCode','status','version','supplierCode','supplierName','supplierEmail','title','description','constructionNotes','qualityNotes','packingNotes','dependencySnapshot','issuedAt','issuedBy','withdrawnAt','withdrawalReason','createdAt','updatedAt'], properties: {
      ...
      dependencySnapshot: { oneOf: [{ $ref: '#/components/schemas/TechPackDependencySnapshot' }, { type: 'null' }] }, issuedAt: dateOrNull(), issuedBy: nullable(160), withdrawnAt: dateOrNull(), withdrawalReason: nullable(500), createdAt: { type: 'string', format: 'date-time' }, updatedAt: { type: 'string', format: 'date-time' },
    } },
