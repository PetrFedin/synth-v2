...
function view(client) {
  return Object.freeze({
    ...
    async getMaterials(codes) {
      ...
      const result = await client.query('SELECT payload FROM materials WHERE code = ANY($1::text[]) ORDER BY code FOR SHARE', [codes]);
    ...
    async saveBom(bom, expectedVersion) {
      ...
      const result = await client.query(
        `UPDATE boms
            SET status = $4, currency = $5, material_cost = $6, labor_cost = $7, overhead_cost = $8,
                logistics_cost = $9, other_cost = $10, total_cost = $11, version = $12, payload = $13::jsonb
...
function bomParameters(bom) {
  return [bom.id, bom.sku, bom.brandId, bom.status, bom.currency, bom.materialCost, bom.laborCost, bom.overheadCost, bom.logisticsCost, bom.otherCost, bom.totalCost, bom.version, JSON.stringify(bom)];
}
