      ...
      try {
        await client.query(
          ...
          [
            ...
            run.periodEnd,
            run.asOfTimestamp,
            run.reportingTimezone,
            run.engineVersion,
            JSON.stringify(run.sourceManifest),
      ...
      try {
        await client.query(
          ...
          [
            binding.id,
            binding.runId,
            binding.kpiDefinitionId,
            binding.releaseEventId,
            binding.activationEventId,
            binding.mappingSetVersion,
    ...
    async insertMappingBinding(binding) {
      try {
        await client.query(
          ...
          [
            binding.id,
            binding.runDefinitionBindingId,
            binding.variableName,
            binding.kpiSourceMappingId,
            binding.verificationEventId,
            binding.createdAt,
            binding.createdBy,
            binding.contentHash,
            JSON.stringify(binding),
    ...
    async listMappingBindings(runDefinitionBindingId) {
      const result = await client.query(
        `SELECT payload
          ...
          WHERE run_definition_binding_id = $1
          ORDER BY variable_name, id`,
        [runDefinitionBindingId],
      );
      return result.rows.map((row) => row.payload);
    },
      ...
      try {
        await client.query(
          ...
          [
            observation.id,
            observation.runId,
            observation.runDefinitionBindingId,
            observation.organisationId,
            observation.periodStart,
            observation.periodEnd,
            observation.asOfTimestamp,
            JSON.stringify(observation.grain),
            observation.grainHash,
            observation.dataState,
            observation.valueNumeric,
            observation.canonicalUom,
            observation.numeratorNumeric,
            observation.denominatorNumeric,
            observation.normalizerK,
            JSON.stringify(observation.componentPayload),
            JSON.stringify(observation.sourceLineage),
      ...
      try {
        await client.query(
          ...
          [
            ...
            result.severity,
            result.resultStatus,
            JSON.stringify(result.observedPayload),
            JSON.stringify(result.expectedContract),
      ...
      try {
        await client.query(
          ...
          [
            ...
            result.runDefinitionBindingId,
            result.observationId,
            result.reconciliationRuleId,
            result.reconciliationRuleVersion,
            result.observedNumeric,
            result.expectedNumeric,
            result.absoluteDifference,
            result.relativeDifference,
            JSON.stringify(result.toleranceContract),
            result.resultStatus,
            JSON.stringify(result.evidence),
            result.evaluatedAt,
            result.contentHash,
    ...
    async insertRestatement(restatement) {
      try {
        await client.query(
          ...
          [
            restatement.id,
            restatement.newRunId,
            restatement.supersededRunId,
            restatement.reasonCode,
            restatement.reason,
            restatement.approvedBy,
            restatement.createdAt,
            restatement.contentHash,
            JSON.stringify(restatement),
          ],
        );
      } catch (error) {
