...
export function canonicalKpiDecimal(value, code = 'KPI_DECIMAL_INVALID') {
  ...
  const precisionDigits = integer.replace(/^0+/, '').length + rawFraction.length;
  invariant(precisionDigits <= KPI_NUMERIC_PRECISION, code, `KPI numeric precision must be <= ${KPI_NUMERIC_PRECISION}`);

  if (integer === '0' && fraction.length === 0) negative = false;
...
function fromScaledBigInt(value) {
  ...
  return canonicalKpiDecimal(`${negative ? '-' : ''}${integer}${fraction ? `.${fraction}` : ''}`);
}
