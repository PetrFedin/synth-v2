...
export function createKpiCalculationRun({
  ...
  const basis = Object.freeze({
    ...
    sourceManifest: requiredNonEmptyObject(sourceManifest, 'KPI_RUN_SOURCE_MANIFEST_INVALID'),
    inputManifestHash,
...
export function createKpiRunRestatement({
  ...
  invariant(Date.parse(newRun.requestedAt) >= Date.parse(supersededRun.requestedAt), 'KPI_RESTATEMENT_TIME_ORDER_INVALID', 'Restatement run cannot predate superseded run request');
...
function assertGovernanceEventNotFromFuture(event, run, code) {
  ...
  invariant(Date.parse(event.createdAt) <= Date.parse(run.requestedAt), code, 'Run cannot bind governance evidence created after the run request');
}
