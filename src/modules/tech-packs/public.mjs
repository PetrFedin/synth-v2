...
export const TECH_PACK_STATUSES = Object.freeze(['draft', 'issued', 'acknowledged', 'superseded', 'withdrawn']);
...
export function issueTechPack(techPack, { catalogSku, bom, measurementChart, approvedSample, actorId, issuedAt }) {
  ...
  assertNotBefore(techPack.updatedAt, at);
  return freezeTechPack({ ...techPack, status: 'issued', version: techPack.version + 1, issuedAt: at, issuedBy: actorId, acknowledgedAt: null, acknowledgement: null, updatedAt: at });
}

export function acknowledgeTechPack(techPack, { supplierCode, acknowledgementReference, acknowledgedBy, notes, acknowledgedAt }) {
  invariant(techPack?.status === 'issued', 'TECH_PACK_NOT_ISSUED', 'Only an issued tech pack can be acknowledged');
  const normalizedSupplierCode = code(supplierCode, 'TECH_PACK_ACK_SUPPLIER_CODE_INVALID');
  invariant(normalizedSupplierCode === techPack.supplierCode, 'TECH_PACK_ACK_SUPPLIER_MISMATCH', 'Acknowledgement supplier does not match the issued tech pack', { expectedSupplierCode: techPack.supplierCode, supplierCode: normalizedSupplierCode });
  const at = timestamp(acknowledgedAt, 'TECH_PACK_ACKNOWLEDGED_AT_INVALID');
  assertNotBefore(techPack.issuedAt, at);
  const acknowledgement = Object.freeze({
    supplierCode: normalizedSupplierCode,
    acknowledgementReference: requiredText(acknowledgementReference, 2, 160, 'TECH_PACK_ACK_REFERENCE_INVALID'),
    acknowledgedBy: requiredText(acknowledgedBy, 2, 160, 'TECH_PACK_ACKNOWLEDGED_BY_INVALID'),
    notes: optionalText(notes, 1000, 'TECH_PACK_ACK_NOTES_INVALID'),
    acknowledgedAt: at,
    issuedTechPackVersion: techPack.version,
  });
  return freezeTechPack({ ...techPack, status: 'acknowledged', version: techPack.version + 1, acknowledgedAt: at, acknowledgement, updatedAt: at });
}

export function createTechPackRevision({ id, issuedTechPack, catalogSku, input, createdAt }) {
  invariant(['issued', 'acknowledged'].includes(issuedTechPack?.status), 'TECH_PACK_NOT_ACTIVE', 'Only an issued or acknowledged tech pack can create a revision');
...
export function supersedeTechPack(techPack, { supersededAt }) {
  invariant(['issued', 'acknowledged'].includes(techPack?.status), 'TECH_PACK_NOT_ACTIVE', 'Only an issued or acknowledged tech pack can be superseded');
...
export function withdrawTechPack(techPack, { reason, withdrawnAt }) {
  invariant(['draft', 'issued', 'acknowledged'].includes(techPack?.status), 'TECH_PACK_NOT_WITHDRAWABLE', 'Only a draft, issued or acknowledged tech pack can be withdrawn');
...
function freezeTechPack(value) { return Object.freeze({ ...value, dependencySnapshot: value.dependencySnapshot ? Object.freeze({ ...value.dependencySnapshot }) : null, acknowledgement: value.acknowledgement ? Object.freeze({ ...value.acknowledgement }) : null }); }
function assertNotBefore(previous, current) { invariant(Number.isFinite(Date.parse(previous)) && Date.parse(current) >= Date.parse(previous), 'TECH_PACK_TIME_ORDER_INVALID', 'Tech pack lifecycle cannot move time backwards', { previous, current }); }
