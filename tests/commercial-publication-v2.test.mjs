...
function projection() {
  return {
    ...
    payload: {
      ...
      technicalSnapshot: {
        legacyEvidence: [{ productSkuId: 'psku:1', catalogSku: 'SKU-1', catalog: { sku: 'SKU-1', status: 'published', version: 4, wholesalePrice: 1000, currency: 'RUB', minimumOrderQuantity: 1, availableQuantity: 10, reservedQuantity: 0 } }],
        product: {
...
test('V2 publication derives Style -> Colorway -> ordered Size/SKU only from immutable projection payload', () => {
  ...
  assert.equal(publication.lines[0].catalogVersion, 4);
  assert.equal(publication.lines[0].unitPrice, 1000);
});

test('V2 publication fails closed if frozen compatibility price snapshot is absent', () => {
  const value = projection();
  value.payload.technicalSnapshot.legacyEvidence = [];
  assert.throws(() => createProjectionBackedCommercialPublication({ id: 'publication:1', collection, commercialProjection: value, publishedAt: at }), (error) => error?.code === 'COMMERCIAL_PUBLICATION_LEGACY_PRICE_SNAPSHOT_REQUIRED');
});
