...
test('cost close and physical cost writer share one transaction serialization key', () => {
  assert.match(migration, /pg_advisory_xact_lock\(hashtextextended\(NEW\.order_commit_snapshot_id, 0\)\)/);
  assert.match(fulfillmentStore, /pg_advisory_xact_lock\(hashtextextended\(\$1, 0\)\)/);
