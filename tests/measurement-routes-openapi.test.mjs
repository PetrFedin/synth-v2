...
function fixture() {
  ...
  return {
    calls,
    routes: createWholesaleRoutes({
      platform: inert, catalog: inert, materials: inert, boms: inert, measurements,
      partners: inert, collaboration: inert, orders: inert, notifications: inert, workspace: inert,
    }),
  };
}
...
const editable = {
  unit: 'cm',
  baseSizeCode: 'M',
  sizes: [{ code: 'S', label: 'Small' }, { code: 'M', label: 'Medium' }],
  points: [{
    pointCode: 'CHEST',
    name: 'Half chest',
    description: null,
    toleranceMinus: 0.5,
    tolerancePlus: 0.5,
    measurements: [{ sizeCode: 'S', value: 48 }, { sizeCode: 'M', value: 51 }],
  }],
  notes: null,
};

test('Measurement Charts routes expose bounded reads and governed mutations', async () => {
...
test('transport rejects derived and unknown fields at every measurement matrix level', () => {
  const { routes, calls } = fixture();
  const create = matchWholesaleRoute(routes, 'POST', '/v2/measurements');
  assert.throws(
    () => create.execute({ actorId: 'owner-user', commandId: 'cmd-top', query: {}, params: create.params, body: { sku: 'STYLE-001', ...editable, skuVersion: 9 } }),
    { code: 'HTTP_BODY_FIELD_UNKNOWN' },
  );
  assert.throws(
    () => create.execute({ actorId: 'owner-user', commandId: 'cmd-size', query: {}, params: create.params, body: { sku: 'STYLE-001', ...editable, sizes: [{ ...editable.sizes[0], position: 1 }] } }),
    (error) => error?.code === 'HTTP_BODY_FIELD_UNKNOWN' && error.details?.field === 'sizes' && error.details?.index === 0,
  );
  assert.throws(
    () => create.execute({ actorId: 'owner-user', commandId: 'cmd-point', query: {}, params: create.params, body: { sku: 'STYLE-001', ...editable, points: [{ ...editable.points[0], baseValue: 51 }] } }),
    (error) => error?.code === 'HTTP_BODY_FIELD_UNKNOWN' && error.details?.field === 'points' && error.details?.index === 0,
  );
  assert.throws(
    () => create.execute({
      actorId: 'owner-user', commandId: 'cmd-value', query: {}, params: create.params,
      body: { sku: 'STYLE-001', ...editable, points: [{ ...editable.points[0], measurements: [{ sizeCode: 'M', value: 51, deltaFromPrevious: 3 }] }] },
    }),
    (error) => error?.code === 'HTTP_BODY_FIELD_UNKNOWN' && error.details?.pointIndex === 0 && error.details?.valueIndex === 0,
  );
...
test('one authoritative OpenAPI document includes Materials, BOM and Measurement Charts', () => {
  ...
  assert.ok(specification.paths['/measurements'].post);
  ...
  assert.equal(specification.components.schemas.MeasurementPointInput.additionalProperties, false);
  assert.equal(specification.components.schemas.MeasurementValueInput.properties.deltaFromPrevious, undefined);
  assert.equal(specification.components.schemas.MeasurementChartUpdate.required.includes('notes'), true);
