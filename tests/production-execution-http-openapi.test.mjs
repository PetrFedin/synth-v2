...
test('Production Execution routes expose bounded reads and explicit lifecycle commands', async () => {
  ...
  const cases = [
    ['GET', '/v2/production-executions', { query: { status: 'active', limit: '50' } }], ['GET', '/v2/production-executions/EXEC-PO-001', {}], ['POST', '/v2/production-executions/from-production-order/PO-001', { body: {} }], ['POST', '/v2/production-executions/EXEC-PO-001/start', { body: { expectedVersion: 1 } }], ['POST', '/v2/production-executions/EXEC-PO-001/milestones/complete', { body: { expectedVersion: 2, milestoneCode: 'materials-ready', notes: null } }], ['POST', '/v2/production-executions/EXEC-PO-001/milestones/block', { body: { expectedVersion: 3, milestoneCode: 'cutting-complete', reason: 'Cutting marker unavailable' } }], ['POST', '/v2/production-executions/EXEC-PO-001/milestones/resolve', { body: { expectedVersion: 4, milestoneCode: 'cutting-complete', notes: 'Replacement marker received' } }], ['POST', '/v2/production-executions/EXEC-PO-001/cancel', { body: { expectedVersion: 5, reason: 'Factory capacity withdrawn' } }],
  ];
  for (let index = 0; index < cases.length; index += 1) { const [method, path, input] = cases[index]; const route = routeFor(routes, method, path); await route.execute({ actorId: 'owner-1', commandId: `command-${index}`, body: input.body ?? {}, query: input.query ?? {}, params: route.params }); }
...
test('OpenAPI 1.18 documents the governed Production Execution calendar without dangling schema references', () => {
