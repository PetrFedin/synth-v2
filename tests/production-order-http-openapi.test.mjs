...
function route(routes, method, path) { const value = routes.find((candidate) => candidate.method === method && candidate.pattern.test(path)); assert.ok(value, `${method} ${path}`); return value; }

test('Production Order routes expose reads and strict lifecycle commands', async () => {
  const calls = [];
  const service = {
    pageForActor: async (...args) => (calls.push(['page', ...args]), { items: [], nextCursor: null }), getForActor: async (...args) => (calls.push(['get', ...args]), { productionOrderNumber: args[1] }), createFromAllocation: async (...args) => (calls.push(['create', ...args]), { productionOrderNumber: 'PO-1' }), issue: async (...args) => (calls.push(['issue', ...args]), { productionOrderNumber: args[2], status: 'issued' }), confirm: async (...args) => (calls.push(['confirm', ...args]), { productionOrderNumber: args[2], status: 'confirmed' }), cancel: async (...args) => (calls.push(['cancel', ...args]), { productionOrderNumber: args[2], status: 'cancelled' }),
  };
  ...
  assert.throws(() => route(routes, 'POST', '/v2/production-orders/from-allocation/RFQ-1').execute({ commandId: 'bad', actorId: 'owner-1', params: ['RFQ-1'], query: {}, body: { productionOrderNumber: 'FORGED' } }), { code: 'HTTP_BODY_FIELD_UNKNOWN' });
});
...
test('OpenAPI 1.18 documents immutable Production Orders without dangling schema references', () => {
  ...
  for (const path of ['/production-orders','/production-orders/{productionOrderNumber}','/production-orders/from-allocation/{rfqCode}','/production-orders/{productionOrderNumber}/issue','/production-orders/{productionOrderNumber}/confirm','/production-orders/{productionOrderNumber}/cancel']) assert.ok(specification.paths[path], path);
