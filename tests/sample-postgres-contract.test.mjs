...
test('Samples lifecycle constraints prevent impossible normalized states', () => {
  assert.match(sql, /payload ->> 'sampleCode' = sample_code/);
  assert.match(sql, /payload ->> 'status' = status/);
  ...
  assert.match(sql, /source_sample_code IS NULL AND payload -> 'sourceSampleCode' = 'null'::jsonb/);
});
