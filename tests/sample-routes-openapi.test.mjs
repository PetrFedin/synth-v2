...
function fixture() {
  const calls = [];
  const samples = {
    pageForActor: async (...args) => { calls.push(['page', ...args]); return { items: [], referenceTime: '2026-08-04T12:00:00.000Z', nextCursor: null }; }, getForActor: async (...args) => { calls.push(['get', ...args]); return { sampleCode: args[1] }; }, createSample: async (...args) => { calls.push(['create', ...args]); return args[2]; }, updateSample: async (...args) => { calls.push(['update', ...args]); return { sampleCode: args[2], ...args[3] }; }, requestSample: async (...args) => { calls.push(['request', ...args]); return { sampleCode: args[2], status: 'requested' }; }, startProduction: async (...args) => { calls.push(['production', ...args]); return { sampleCode: args[2], status: 'in-production' }; }, receiveSample: async (...args) => { calls.push(['receive', ...args]); return { sampleCode: args[2], status: 'received' }; }, decideSample: async (...args) => { calls.push(['decision', ...args]); return { sampleCode: args[2], status: args[3].decision }; }, cancelSample: async (...args) => { calls.push(['cancel', ...args]); return { sampleCode: args[2], status: 'cancelled' }; }, createNextRound: async (...args) => { calls.push(['next', ...args]); return { sampleCode: args[3].sampleCode, status: 'draft' }; },
  };
...
test('Samples routes cover the complete lifecycle without generic transition ambiguity', async () => {
  ...
  const cases = [
    ['GET', '/v2/samples', { query: { overdue: 'true' } }], ['GET', '/v2/samples/SMP-STYLE-001-FIT-R01', {}], ['POST', '/v2/samples', { body: { sampleCode: 'SMP-STYLE-001-FIT-R01', sku: 'STYLE-001', sampleType: 'fit', round: 1, ...editable } }], ['PATCH', '/v2/samples/SMP-STYLE-001-FIT-R01', { body: { expectedVersion: 1, ...editable } }], ['POST', '/v2/samples/SMP-STYLE-001-FIT-R01/request', { body: { expectedVersion: 2 } }], ['POST', '/v2/samples/SMP-STYLE-001-FIT-R01/start-production', { body: { expectedVersion: 3 } }], ['POST', '/v2/samples/SMP-STYLE-001-FIT-R01/receive', { body: { expectedVersion: 4, receivedQuantity: 1, condition: 'accepted', trackingReference: null, notes: null } }], ['POST', '/v2/samples/SMP-STYLE-001-FIT-R01/decision', { body: { expectedVersion: 5, decision: 'rejected', notes: 'Fit failed' } }], ['POST', '/v2/samples/SMP-STYLE-001-FIT-R01/cancel', { body: { expectedVersion: 3, reason: 'Supplier cannot meet the required date' } }], ['POST', '/v2/samples/SMP-STYLE-001-FIT-R01/next-round', { body: { expectedVersion: 6, sampleCode: 'SMP-STYLE-001-FIT-R02', dueAt: '2026-08-28T12:00:00.000Z', notes: 'Correct fit' } }],
  ];
  for (let index = 0; index < cases.length; index += 1) { const [method, path, input] = cases[index]; const route = matchWholesaleRoute(routes, method, path); assert.ok(route, `${method} ${path}`); await route.execute({ actorId: 'owner-user', commandId: `cmd-${index}`, body: input.body ?? {}, query: input.query ?? {}, params: route.params }); }
...
test('authoritative OpenAPI 1.18 documents every Samples command and strict schema', () => {
