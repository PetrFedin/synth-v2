...
function serviceSpy() {
  ...
  return { calls, service: {
    supplierPageForActor: capture('supplierPageForActor'), supplierGetForActor: capture('supplierGetForActor'), createSupplier: capture('createSupplier'), updateSupplier: capture('updateSupplier'), qualifySupplier: capture('qualifySupplier'), suspendSupplier: capture('suspendSupplier'), archiveSupplier: capture('archiveSupplier'), rfqPageForActor: capture('rfqPageForActor'), rfqGetForActor: capture('rfqGetForActor'), createRfq: capture('createRfq'), updateRfq: capture('updateRfq'), issueRfq: capture('issueRfq'), upsertQuote: capture('upsertQuote'), awardRfq: capture('awardRfq'), allocateRfq: capture('allocateRfq'), cancelRfq: capture('cancelRfq'),
  } };
}
...
test('sourcing routes expose the full supplier and RFQ mutation lifecycle', async () => {
  const spy = serviceSpy(); const routes = createSourcingRoutes({ sourcing: spy.service });
  const expectations = [['POST','/v2/suppliers'],['PATCH','/v2/suppliers/FACTORY-A'],['POST','/v2/suppliers/FACTORY-A/qualify'],['POST','/v2/suppliers/FACTORY-A/suspend'],['POST','/v2/suppliers/FACTORY-A/archive'],['GET','/v2/suppliers'],['GET','/v2/suppliers/FACTORY-A'],['POST','/v2/rfqs'],['PATCH','/v2/rfqs/RFQ-001'],['POST','/v2/rfqs/RFQ-001/issue'],['POST','/v2/rfqs/RFQ-001/quotes'],['POST','/v2/rfqs/RFQ-001/award'],['POST','/v2/rfqs/RFQ-001/allocate'],['POST','/v2/rfqs/RFQ-001/cancel'],['GET','/v2/rfqs'],['GET','/v2/rfqs/RFQ-001']];
...
test('OpenAPI 1.18 publishes strict supplier, quotation, award and guarded production allocation contracts', () => {
  ...
  assert.equal(quote.additionalProperties, false); assert.ok(quote.required.includes('unitPriceMinor')); assert.ok(quote.required.includes('validUntil'));
  const allocation = wholesaleV2ExtendedOpenApi.components.schemas.RfqAllocationInput;
  assert.ok(allocation.required.includes('purchaseOrderNumber')); assert.ok(allocation.required.includes('productionStartAt')); assert.equal(allocation.properties.techPackCode, undefined);
  const resultAllocation = wholesaleV2ExtendedOpenApi.components.schemas.RfqAllocation;
  assert.ok(resultAllocation.required.includes('techPackCode')); assert.ok(resultAllocation.required.includes('techPackAcknowledgementReference'));
