...
test('Tech Pack routes expose issue, acknowledgement, revision and withdrawal commands', async () => {
  ...
  const cases = [
    ['GET', '/v2/tech-packs', { query: { status: 'acknowledged' } }],
...
test('authoritative OpenAPI 1.14 documents supplier acknowledgement and guarded production allocation', () => {
  ...
  assert.equal(allocation.required.includes('techPackCode'), true);
  assert.equal(allocation.required.includes('techPackAcknowledgementReference'), true);
