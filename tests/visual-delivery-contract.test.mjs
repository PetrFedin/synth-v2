...
test('the shell loads Tech Packs and Omnidata V14 after every implemented product workspace', async () => {
  const html = await readFile(path.join(publicDir, 'index.html'), 'utf8');
  assert.match(html, /meta name="syntha-build" content="visual-20260805-14"/);
  for (const asset of [
    'tech-packs.css?v=industrial-20260805-3',
    ...
    'omnidata-v13.js?v=visual-20260804-13',
    'omnidata-v14.js?v=visual-20260805-14',
  ]) assert.ok(html.includes(asset), asset);

  const styles = [...html.matchAll(/<link\s+[^>]*rel="stylesheet"[^>]*href="([^"]+)"/g)]
    .map((match) => new URL(match[1], 'http://syntha.local').pathname);
  assert.deepEqual(styles.slice(-9), [
    '/omnidata-v8.css',
...
test('the standalone server prevents stale caching of active V14 assets', async () => {
  ...
  await withServer(handler, async (base) => {
    for (const asset of [
      '/omnidata-v14.css?v=visual-20260805-14',
      '/omnidata-v14-extensions.css?v=visual-20260805-14',
      '/ui/omnidata-v14.js?v=visual-20260805-14',
    ]) {
