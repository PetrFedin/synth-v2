...
function emptyWorkspace(overrides = {}) {
  return {
    ...
    catalogSkus: [], showrooms: [], cycles: [], selections: [], orders: [], deals: [], calendar: [], ...overrides,
  };
}

test('workspace service validates limits before invoking the reader', async () => {
  let reads = 0;
  const service = createWorkspaceQueryService({ reader: { async readForActor() { reads += 1; return emptyWorkspace(); } } });
  for (const limit of ['0', '501', '1.5', '+1', ' 10 ', -1, 501]) {
    await assert.rejects(
      () => service.loadForActor('actor-1', { limit }),
      (error) => error.code === 'WORKSPACE_LIMIT_INVALID' && error.details.min === 1 && error.details.max === 500,
    );
  }
...
test('reader fetches limit plus one and reports truncated sections without returning the sentinel row', async () => {
  ...
  const client = {
    async query(sql, params = []) {
      ...
      if (sql.includes('SELECT organisation_id')) return { rows: [
  ...
  assert.equal(workspace.pageInfo.limit, 2);
  assert.equal(workspace.pageInfo.hasMore, true);
  assert.deepEqual(workspace.pageInfo.truncatedSections, ['memberships']);
  assert.equal(typeof workspace.pageInfo.nextCursors.memberships, 'string');
...
test('reader rejects unsafe direct limits before checking out a connection', async () => {
  ...
  for (const limit of [undefined, 0, 501, 1.5, '20']) {
    await assert.rejects(() => reader.readForActor('actor-1', { limit }), (error) => error.code === 'WORKSPACE_LIMIT_INVALID');
  }
...
test('workspace HTTP route forwards only the declared limit query', async () => {
  const calls = [];
  const services = {
    platform: {}, catalog: {}, partners: {}, collaboration: {}, orders: {}, notifications: {},
    workspace: { async loadForActor(actorId, options) { calls.push({ actorId, options }); return emptyWorkspace(); } },
  };
