...
const collections = ['memberships','organisations','relationships','invitations','campaigns','collections','catalogSkus','showrooms','cycles','selections','orders','deals','calendar'];

test('workspace output is deterministic and deeply immutable', async () => {
  const reader = {
    async readForActor() {
      return {
        memberships: [{ id: 'm2', organisationId: 'shop-2', userId: 'u1' }, { id: 'm1', organisationId: 'brand-1', userId: 'u1' }],
        organisations: [{ id: 'shop-2', type: 'shop', name: 'Zulu' }, { id: 'brand-1', type: 'brand', name: 'Alpha' }],
        relationships: [], invitations: [],
        campaigns: [{ id: 'campaign-old', name: 'Old', startsAt: '2026-01-01' }, { id: 'campaign-new', name: 'New', startsAt: '2027-01-01' }],
        collections: [], catalogSkus: [{ sku: 'SKU-10' }, { sku: 'SKU-2' }], showrooms: [], cycles: [],
        selections: [{ id: 'selection-1', lines: [{ sku: 'SKU-1', quantity: 2 }] }], orders: [], deals: [],
        calendar: [{ id: 'late', startsAt: '2027-02-01' }, { id: 'early', startsAt: '2027-01-01' }],
      };
    },
  };
  ...
  assert.equal(Object.isFrozen(workspace.pageInfo), true);
...
test('workspace reader uses one repeatable read-only transaction', async () => {
  ...
  const client = {
    async query(sql, params) {
      ...
      if (/FROM\s+memberships/.test(sql)) return { rows: [{ payload: { organisationId: 'brand-1', organisationType: 'brand', userId: 'user-1', status: 'active' } }] };
      return { rows: [] };
    },
    release() { queries.push({ sql: 'RELEASE' }); },
  };
  const pool = { async connect() { return client; }, async query() { throw new Error('workspace must not use pool.query'); } };
...
test('workspace reader returns the complete empty shape for actors without memberships', async () => {
  const queries = [];
  const client = { async query(sql, params) { queries.push({ sql, params }); return { rows: [] }; }, release() { queries.push({ sql: 'RELEASE' }); } };
  const workspace = await createPostgresWorkspaceReader({ pool: { async connect() { return client; } } }).readForActor('user-without-membership');
  for (const key of collections) assert.deepEqual(workspace[key], []);
  ...
  assert.match(queries[1].sql, /FROM\s+memberships/);
...
test('workspace reader rolls back and releases its connection after a query failure', async () => {
  ...
  const client = {
    async query(sql) { queries.push(sql); if (/FROM\s+memberships/.test(sql)) throw failure; return { rows: [] }; },
    release() { queries.push('RELEASE'); },
  };
  await assert.rejects(() => createPostgresWorkspaceReader({ pool: { async connect() { return client; } } }).readForActor('user-1'), (error) => error === failure);
...
test('workspace service rejects malformed reader results', async () => {
  ...
  await assert.rejects(() => service.loadForActor('user-1'), (error) => error.code === 'WORKSPACE_COLLECTION_INVALID');
});
