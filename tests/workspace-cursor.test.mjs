...
test('workspace cursor rejects cross-section, non-canonical, malformed and wrong-shape values', () => {
  ...
  assert.throws(
    () => encodeWorkspaceCursor({ section: 'orders', position: ['only-one'] }),
    (error) => error.code === 'WORKSPACE_CURSOR_INVALID',
  );
  assert.throws(
    () => encodeWorkspaceCursor({ section: 'catalogSkus', position: [null] }),
    (error) => error.code === 'WORKSPACE_CURSOR_INVALID',
  );
});
...
test('workspace cursor enforces bounded scalar values', () => {
  assert.throws(
    () => encodeWorkspaceCursor({ section: 'catalogSkus', position: ['x'.repeat(513)] }),
    (error) => error.code === 'WORKSPACE_CURSOR_INVALID',
  );
  assert.throws(
    () => encodeWorkspaceCursor({ section: 'collections', position: [{ unsafe: true }, 'collection-1'] }),
    (error) => error.code === 'WORKSPACE_CURSOR_INVALID',
  );
});
