...
function chartInput(sku, overrides = {}) {
  return {
    ...
    points: [
      {
        pointCode: 'CHEST', name: 'Half chest', description: 'Two centimetres below armhole', toleranceMinus: 0.5, tolerancePlus: 0.75,
        measurements: [{ sizeCode: 'S', value: 48.5 }, { sizeCode: 'M', value: 51.5 }, { sizeCode: 'L', value: 54.5 }],
      },
      {
        pointCode: 'BODY-LEN', name: 'Body length', description: null, toleranceMinus: 0.3, tolerancePlus: 0.3,
        measurements: [{ sizeCode: 'S', value: 69.1 }, { sizeCode: 'M', value: 70.2 }, { sizeCode: 'L', value: 71.3 }],
      },
    ],
    notes: 'Initial measuring method',
    ...overrides,
  };
}
...
test('PostgreSQL Measurement Charts preserve matrix integrity, RBAC, versions and events', { skip: !databaseUrl }, async () => {
  ...
  let id = 0; let tick = 0;
  const clock = () => `2026-08-04T14:00:${String(tick++).padStart(2, '0')}.000Z`;
  ...
  try {
    ...
    const measurements = Object.freeze({
      ...createMeasurementService({ measurementStore, clock, nextId }),
    ...
    await platform.grantMembership('member-owner', 'system', createMembership({
      id: 'membership-owner', organisationId: 'brand-measurement', organisationType: 'brand', userId: 'product-owner', role: 'owner', createdAt: clock(),
    }));
    for (const [role, userId] of [['sales', 'sales-user'], ['finance', 'finance-user']]) {
      await platform.grantMembership(`member-${role}`, 'product-owner', createMembership({
        id: `membership-${role}`, organisationId: 'brand-measurement', organisationType: 'brand', userId, role, createdAt: clock(),
      }));
    }

    const campaign = await platform.createCampaign('campaign-create', 'product-owner', {
      brandId: 'brand-measurement', name: 'SS Measurement', season: 'SS28', startsAt: '2027-06-01T00:00:00.000Z', endsAt: '2027-07-01T00:00:00.000Z',
    });
    ...
    const collection = await platform.createCollection('collection-create', 'product-owner', {
      campaignId: campaign.id, brandId: 'brand-measurement', name: 'Main', currency: 'EUR',
    });
    ...
    const skuDraft = await catalog.createSku('sku-create', 'product-owner', {
      sku: 'MEAS-PG-1', collectionId: collection.id, brandId: 'brand-measurement', name: 'Graded Jacket',
      wholesalePrice: 140, currency: 'EUR', minimumOrderQuantity: 2, availableQuantity: 30,
    });
    ...
    await assert.rejects(
      () => measurements.publishMeasurementChart('measurement-stale-publish', 'product-owner', created.sku, { expectedVersion: created.version }),
      { code: 'MEASUREMENT_SKU_SNAPSHOT_STALE' },
    );

    const editable = chartInput(created.sku, {
      notes: 'Rebased after SKU publication',
      points: [
        { ...initial.points[0], measurements: [{ sizeCode: 'S', value: 49 }, { sizeCode: 'M', value: 52.25 }, { sizeCode: 'L', value: 55.5 }] },
        initial.points[1],
      ],
    });
    delete editable.sku;
    ...
    await assert.rejects(
      () => measurements.updateMeasurementChart('measurement-stale-update', 'product-owner', created.sku, { expectedVersion: created.version, ...editable }),
      { code: 'MEASUREMENT_CONCURRENCY_CONFLICT' },
    );
    ...
    const aggregate = (await pool.query(
      `SELECT sku_version, status, unit, base_size_code, version, payload
         FROM measurement_charts WHERE sku = $1`,
      [created.sku],
    )).rows[0];
    assert.deepEqual(
      { sku_version: aggregate.sku_version, status: aggregate.status, unit: aggregate.unit, base_size_code: aggregate.base_size_code, version: aggregate.version },
      { sku_version: publishedSku.version, status: 'published', unit: 'cm', base_size_code: 'M', version: 3 },
    );
    assert.equal(aggregate.payload.points[0].baseValue, 52.25);

    const sizes = await pool.query(
      'SELECT size_code, label, position FROM measurement_chart_sizes WHERE chart_id = $1 ORDER BY position',
      [published.id],
    );
    assert.deepEqual(sizes.rows, [
      { size_code: 'S', label: 'Small', position: 1 },
      { size_code: 'M', label: 'Medium', position: 2 },
      { size_code: 'L', label: 'Large', position: 3 },
    ]);
    const points = await pool.query(
      `SELECT point_code, position, tolerance_minus::text AS tolerance_minus, tolerance_plus::text AS tolerance_plus, base_value::text AS base_value
         FROM measurement_points WHERE chart_id = $1 ORDER BY position`,
      [published.id],
    );
    ...
    const values = await pool.query(
      `SELECT point_code, size_code, value::text AS value, delta_from_previous::text AS delta_from_previous
         FROM measurement_values WHERE chart_id = $1 AND point_code = 'CHEST'
        ORDER BY CASE size_code WHEN 'S' THEN 1 WHEN 'M' THEN 2 ELSE 3 END`,
      [published.id],
    );
    ...
    assert.deepEqual(
      (await pool.query("SELECT event_type FROM outbox_events WHERE event_type LIKE 'measurement.%' ORDER BY event_type")).rows.map((row) => row.event_type),
      ['measurement.created', 'measurement.published', 'measurement.updated'],
    );
  } finally {
    await pool.end();
  }
